...
test_that("output from quick_map() are as expected",{
  expect_equal("ggplot" %in% class(quick_map(example, "price")), TRUE)
  expect_equal("ggplot" %in% class(quick_map(example, "price", theme = 2)), TRUE)
  expect_equal("ggplot" %in% class(quick_map(example, "price", theme = "Candy")), TRUE)
  expect_equal("ggplot" %in% class(quick_map(example, "price", verbose = FALSE)), TRUE)
  expect_equal("ggplot" %in% class(quick_map(example, "price", width_plot = 10)), TRUE)
  expect_equal("ggplot" %in% class(quick_map(example, "price", reverse_palette = TRUE)), TRUE)
  expect_equal("ggplot" %in% class(quick_map(example, "price", col_border = "white")), TRUE)
  expect_equal("ggplot" %in% class(quick_map(example2, "price", col_breaks = c(1, 5, 150,500, 1e6))), TRUE)
  expect_equal("ggplot" %in% class(quick_map(example, "test",col_na = "black")), TRUE)
})
